...
export function Header() {
  ...
  return (
    <>
        ...
        <div className="flex justify-between items-center h-14 xs:h-16 sm:h-20">
          ...
          <Link href="/" className="flex items-center group flex-shrink-0">
            ...
            <div className="ml-2 sm:ml-4 hidden xs:block">
              ...
              <div className="text-xs text-primary-600 font-medium -mt-1 hidden sm:block" suppressHydrationWarning>
                {t('header.tagline')}
              </div>
            </div>
      ...
      {/* Mobile Navigation Overlay - Outside header for full screen overlay */}
      <>
        {/* Backdrop */}
        <div 
        ...
        {/* Sidebar Menu */}
        <div className={`fixed top-0 right-0 h-full w-80 max-w-[90vw] bg-white shadow-2xl z-[100] md:hidden transform transition-transform duration-300 ease-in-out ${
          isMenuOpen ? 'translate-x-0' : 'translate-x-full'
            ...
            <div className="flex flex-col h-full overflow-y-auto pb-20">
                ...
                <div className="space-y-1">
                  <Link 
                    ...
                    className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${
                      isActive('/') ? 'bg-accent-50 text-accent-600' : 'text-gray-700 hover:bg-gray-50'
                    }`}
                    onClick={() => setIsMenuOpen(false)}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <Link 
                    href="/products" 
                    className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${
                      isActive('/products') ? 'bg-accent-50 text-accent-600' : 'text-gray-700 hover:bg-gray-50'
                    }`}
                    onClick={() => setIsMenuOpen(false)}
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="py-2">
                    ...
                    <div className="space-y-1 ml-4">
                      <Link
                        ...
                        className="flex items-center space-x-3 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg transition-colors"
                        onClick={() => setIsMenuOpen(false)}
                      >
                      ...
                      >
                        ...
                        <span>{t('products.categories.beauty_perfumes')}</span>
                      </Link>
                    </div>
                  </div>
                  ...
                  <Link 
                    href="/cart" 
                    className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors ${
                      isActive('/cart') ? 'bg-accent-50 text-accent-600' : 'text-gray-700 hover:bg-gray-50'
                    }`}
                    onClick={() => setIsMenuOpen(false)}
                  >
                    <div className="relative">
              ...
              <div className="border-t border-gray-200 p-4">
                {/* Contact Button */}
                <button 
                  className="w-full flex items-center justify-center space-x-2 bg-gradient-to-r from-accent-500 to-accent-600 text-white px-4 py-3 rounded-lg font-medium hover:from-accent-600 hover:to-accent-700 transition-colors shadow-lg"
                  onClick={() => {
                    setIsMenuOpen(false);
                    scrollToContact();
                  }}
